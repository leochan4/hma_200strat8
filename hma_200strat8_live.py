...
#change PnL calculations, as right now the calculation gives negative calculations
#change loop logic so that if the bot crashes, it only retries to connect for an hour. 
#max email send should be 60.
#change IBKR password to remove capital letters
...
def log_trade(entry_time, action, entry_type, entry_price, exit_price, filename='trade_log.csv'):
    ...
    pnl = round(exit_price - entry_price, 4) if action == 'SELL' else round(entry_price - exit_price, 4)
                ...
                if now.minute % 15 == 0 and now.second < 15:
                    ...
                    if current_position == 1 and latest['oc_pct_change'] < -threshold:
                        ...
                        if filled:
                            ...
                            log_trade(now, 'SELL', entry_type, entry_fill_price, exit_fill_price, log_file)
